...
```
docker --tls build  -t local-hadoop-2.7.0 .
```
## Start a container
...
```
docker --tls run -i -t local-hadoop-2.7.0 /etc/bootstrap.sh -bash
```
 ...
 * Get containers IP address
	* To get containers IP address we need CONTAINER_ID. To get container id use the following command which should list all running containers and its ID
	`docker --tls ps`
	* Use the following command to get containers IP address (where CONTAINER_ID is the container id of local-hadoop-2.7.0 (or prasanthj/docker-hadoop if pulled from docker hub) image)
